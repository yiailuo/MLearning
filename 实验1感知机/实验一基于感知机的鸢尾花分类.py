...
iris = load_iris()
print("========分类标签=============")
print(iris.feature_names)  # 输出4个分类标签
print(iris.data)  # 输出iris数据集的规模，150行数据，4个属性
print("=========数据规模============")
print(iris.data.shape)
print("=========数据的类型Setosa，Versicolour，Virginica============")
print(iris.target)  # 数据的3种类型（Setosa，Versicolour，Virginica）
print("=====================")
# # 鸢尾花数据集
...
# -------------数据为Setosa 山鸢尾+Virginica 维吉尼亚鸢尾---------
# 先按行切割数据再按列拼接数据
# X_1 = np.c_[np.ones(50), iris.data[:50, 2:4]]
# X_2 = np.c_[np.ones(50), iris.data[100:150, 2:4]]
# X = np.r_[X_1, X_2]
# # 得到真值T，从数据集中得到真值
# T_1 = iris.target[:50]
# T_2 = iris.target[100:150]
# T = np.r_[T_1, T_2].reshape(100, 1)
# # 将T中等于0的赋值为-1，等于2的赋值为1，以契合sign函数
# T[T == 0] = -1
# T[T == 2] = 1
#
...
# ---------三个输入：Setosa 山鸢尾+Virginica 维吉尼亚鸢尾的花瓣长度、花瓣宽度、花萼长度----------
X_1 = np.c_[np.ones(50), iris.data[:50, 2:4], iris.data[:50, 0:2]]
X_2 = np.c_[np.ones(50), iris.data[100:150, 2:4], iris.data[100:150, 0:2]]
X = np.r_[X_1, X_2]
# 得到真值T，从数据集中得到真值
T_1 = iris.target[:50]
T_2 = iris.target[100:150]
